...
function getUsersWithReservations() {
    return knex('users')
        ...
        .select('users.id', 'username', 'roles', 'reservations.id AS reservation_id', 'bikes.id AS bike_id', 'model', 'color', 'location', 'reservations.active AS reservation_active', 'reserved_from', 'reserved_to')
        .orderBy('users.id')
